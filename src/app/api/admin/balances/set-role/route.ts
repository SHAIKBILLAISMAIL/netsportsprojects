...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!userId || !role) {
      return NextResponse.json(
        { error: 'userId and role are required', code: 'INVALID_INPUT' },
        { status: 400 }
      );
    }
    ...
    if (!['user', 'agent', 'admin'].includes(role)) {
      return NextResponse.json(
        { error: 'Role must be user, agent, or admin', code: 'INVALID_ROLE' },
        { status: 400 }
      );
    }
    ...
    console.error('POST /api/admin/balances/set-role error:', error);
    return NextResponse.json(
